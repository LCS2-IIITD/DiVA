...
// Add canvas
let container = document.getElementById('graph-view-inner-single')
  ...
  const nodes = items.map(function(d){
    ...
    out_edges.map(function(e){
      if(e===''){
        ...
        data_single.links.push({ source: d.id, target: e })
      }
    })
    return node_to_return
  })
  data_single.nodes = data_single.nodes.concat(nodes)
  if (finished) {
    chroma_colors = chroma.scale([chroma('#000088').brighten(3).hex(), '#000088']).mode('lch').colors(max_degree);
    d3.forceSimulation(data_single.nodes)
      .force("boundary", forceBoundary(0,0,250000,100000).strength(0.5))
      .force("charge", d3.forceManyBody().strength(-500))
      .force("link", d3.forceLink(data_single.links).id(function(d){ return d.id }).distance(50))
      .on('tick', ticked)
      .on('end', ended);
  }
};
...
function ended(){
    data_single.nodes.map( function(node){
      if(node.x > max_x){
        max_x = node.x;
      }
    ...
    let colors = [];
    let ref = {}
    ref.x = data_single.nodes[0].x
    ref.y = data_single.nodes[0].y;

    data_single.nodes.map( function(d){
    ...
    for(let i=0; i<data_single.links.length; i++){
      let sourceVertex = new THREE.Vector3(data_single.links[i].source.x-ref.x,data_single.links[i].source.y-ref.y,0)
      linesGeometry.vertices.push(sourceVertex);
      let targetVertex = new THREE.Vector3(data_single.links[i].target.x-ref.x,data_single.links[i].target.y-ref.y,0)
